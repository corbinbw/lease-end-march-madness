          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs sm:text-sm font-bold">{pickedMatches}/{totalMatches} picks ({completionPercent}%)</span>
              </div>
              <div className="w-full bg-navy-600 rounded-full h-2 sm:h-2.5">
                <div 
                  className={`h-2 sm:h-2.5 rounded-full transition-all duration-500 ${isComplete ? 'bg-emerald-400' : 'bg-amber-400'}`}
            ...
            {canPick && (
              <button
                ...
                className={`w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-sm sm:text-lg transition-all ${
                  isComplete 
      ...
      {isSubmitted && (
        <div className="card p-3 sm:p-4 border-0 shadow-md bg-gradient-to-r from-emerald-600 to-emerald-500 text-white">
          <div className="flex items-center justify-center gap-2 sm:gap-3">
            ...
            <span className="font-bold text-base sm:text-lg">Bracket Submitted!</span>
...
function RegionBracket({
  ...
  return (
    <div className="card overflow-hidden border-0 shadow-md">
      {/* Region Header */}
      <div className={`${colors.bg} text-white px-4 py-2 sm:py-3`}>
        <h3 className="font-bold text-center text-sm sm:text-base">{REGION_LABELS[region]}</h3>
      </div>
      
      {/* Bracket Grid */}
      <div className="p-2 sm:p-3 overflow-x-auto bg-white dark:bg-navy-900">
        <div className="flex gap-1 sm:gap-2 min-w-[600px] sm:min-w-[680px]">
          {regionalRounds.map((round, roundIdx) => {
            ...
            return (
              <div key={round} className="flex-1">
                <div className="text-[9px] sm:text-[10px] font-bold text-navy-400 dark:text-navy-500 text-center mb-1 sm:mb-2 uppercase tracking-wider">
                  {round === 'R64' ? 'R64' : round === 'R32' ? 'R32' : round === 'S16' ? 'S16' : 'E8'}
                </div>
                <div className={`space-y-0.5 sm:space-y-1 flex flex-col ${roundIdx === 0 ? '' : 'justify-around h-full'}`}>
...
function FinalRounds({
  ...
  return (
    <div className="card p-4 sm:p-6 border-0 shadow-md bg-gradient-to-br from-white to-navy-50 dark:from-navy-900 dark:to-navy-800">
      <div className="text-center mb-4 sm:mb-6">
        <span className="inline-flex items-center bg-navy-800 dark:bg-navy-700 text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-xl font-bold text-sm sm:text-lg">
          🏆 Final Four & Championship
        </span>
      </div>
      ...
      <div className="flex flex-col items-center gap-4 sm:gap-6">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-4 sm:gap-6 w-full">
          {f4Matches.map((match, i) => (
            <div key={match.id} className="w-full sm:w-64">
...
function MatchCard({
  ...
  const renderEntrant = (entrant: EntrantInfo | null) => {
    if (!entrant) {
      return (
        <div className="flex items-center px-1.5 sm:px-2 py-1 sm:py-1.5 border border-dashed border-navy-200 dark:border-navy-600 rounded text-navy-400 dark:text-navy-500 text-[10px] sm:text-xs">
    ...
    let classes = 'flex items-center justify-between px-1.5 sm:px-2 py-1 sm:py-1.5 border rounded text-[10px] sm:text-xs transition-all '
    ...
    return (
      <div className={classes} onClick={() => clickable && onPick(match.id, entrant.id)}>
        <div className="flex items-center gap-1 sm:gap-1.5 overflow-hidden">
          <span className="bg-navy-700 dark:bg-navy-600 text-white w-4 h-4 sm:w-5 sm:h-5 rounded flex items-center justify-center text-[8px] sm:text-[10px] font-bold shrink-0">
            {entrant.seed}
          </span>
        ...
        <div className="flex items-center gap-0.5 shrink-0 ml-1">
          ...
          {isActualWinner && <span className="text-[10px] sm:text-xs">🏆</span>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-lg border-l-4 ${regionColor} ${isChamp ? 'bg-navy-100 dark:bg-navy-800 border border-navy-200 dark:border-navy-600' : 'bg-navy-50 dark:bg-navy-800/50'} p-1 sm:p-1.5 space-y-0.5 sm:space-y-1`}>
      {renderEntrant(left)}
      {!compact && <div className="text-center text-[8px] sm:text-[9px] text-navy-400 dark:text-navy-500 font-semibold">VS</div>}
      {renderEntrant(right)}
    </div>
  )
}
